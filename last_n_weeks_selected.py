...
def last_n_weeks_selected(path_to_dataframe_in_csv, date_column_name, normalized_date_column_name,
    ...
    data_frame = date_to_seconds(path_to_dataframe_in_csv, date_column_name, normalized_date_column_name,
                                 unit_of_time='weeks')
    ...
    data_frame.loc[
        data_frame[normalized_date_column_name] > data_frame[
            normalized_date_column_name].max() - n_weeks_selected, 'Weeks Marker'] = data_frame[
        normalized_date_column_name].apply(lambda week_number: '{} last'.format(week_number % 4))

    data_frame.loc[
        (data_frame[normalized_date_column_name] > data_frame[
            normalized_date_column_name].max() - 2*n_weeks_selected) & (data_frame[normalized_date_column_name] <= data_frame[
            normalized_date_column_name].max() - n_weeks_selected), 'Weeks Marker'] = data_frame[
        normalized_date_column_name].apply(lambda week_number: '{} previous'.format(week_number % 4))
    ...
    marker_list = ['0 last', '0 previous', '1 last', '1 previous', '2 last', '2 previous', '3 last', '3 previous']
...
last_n_weeks_selected('kauia_dataset_excluded_extras.csv', 'Transaction Date', 'Lala', 'Transaction ID', 'Member ID', 8,
                  product_column='Product Name', user_column='Member ID', output_file='last_n_weeks_selected_8')
    # .to_csv(
    # 'to_weeks_devision_user.csv')
